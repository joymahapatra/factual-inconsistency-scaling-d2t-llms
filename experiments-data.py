...
if __name__ == "__main__":
    ...
    """data collecting"""    
    entire_data = pd.read_csv("./input_data_scaling.tsv", sep='\t', index_col=None, header='infer', encoding="utf-8")
    # dataset_name_list = ['DART', 'E2E', 'ViGGO', 'WebNLG', 'WikiTableText']
    dataset_name_list = ['DART', 'E2E', 'WikiTableText']
    ...
    """storage allocation"""
    os.makedirs("./data_scaling_results", exist_ok=True)
    
    for i_dataset in dataset_name_list:
        data = entire_data[entire_data['Dataset']==i_dataset.lower()]
        global_tag = (i_dataset + "_" + metric_name).lower()
        all_store = utils.Store2DResult(name=f"{global_tag}_all_store", store_dir_path="data_scaling_results")
        if len(data) == 0:
            print(f"No entry for {i_dataset} datset.")
            continue
        given_models = list(set(data.columns).intersection(set(all_models)))
        given_models.sort()
        num_models = len(given_models)
        fig, axs = plt.subplots(nrows=1, ncols=len(given_models), figsize=(8, 1.75))
        for idx, model_t in enumerate(given_models):
            xdata = data['Size'].to_numpy()
            xdata = xdata + 1e-10 # prevent underflow
            ydata = data[model_t].to_numpy()
            ...
            for scaling_type in [exp_scaling_law, pow_scaling_law]:
                print(f"We are with {model_t} and {scaling_type.__name__}.")
                popt, pconv = curve_fit(scaling_type, xdata, ydata, p0=[1.0, 1.0, 0.15], maxfev=5000, method='trf', loss='huber')
                all_store.data[model_t][f"rate/expn_{scaling_type.__name__}"] = f"{popt[0]:1.2e}"
                all_store.data[model_t][f"constant_{scaling_type.__name__}"] = f"{popt[1]:1.2e}"
                all_store.data[model_t][f"intercept_{scaling_type.__name__}"] = f"{popt[2]}"
                # print(*popt, pconv)
                ...
                all_store.data[model_t][f"mean_error_{scaling_type.__name__}"] = f"{pp_mean:1.2e}" # {pp_mean:1.2e} ($\pm${pp_std:1.1e})
                all_store.data[model_t][f"std_error_{scaling_type.__name__}"] = f"{pp_std:1.2e}"
                ...
                all_store.data[model_t][f"ftest_pval_{scaling_type.__name__}"] = f_test_p
                """<end f-test>"""
                ...
                all_store.data[model_t][f"shwlilk_pval_{scaling_type.__name__}"] = shapiro_p
                ...
                axs[idx].set_title(fr"\textbf{{{model_t} (w/ {i_dataset})}}", loc='center')
                # axs[idx].axis("off")
                # axs[idx].set_yticks([])
                # axs[idx].set_yticklabels([])
                axs[idx].set_xlabel(r'Fine-tune Data Size (\%) $\rightarrow$')
                ...
                # axs[idx].xaxis.set_major_formatter(FormatStrFormatter('%.1f'))
            ...
            all_store.data[model_t][f"vuong_pval_{scaling_type.__name__}"] = vuong_p
        # fig.supxlabel('')
        fig.supylabel(fr'Factual Inconsistency w/ \textsc{{{metric_name}}} $\rightarrow$', fontsize=SMALL_SIZE, x=0.01)
        fig.tight_layout()
        fig.savefig(f"data_scaling_results/{global_tag}_plot.pdf", format="pdf", dpi=200, bbox_inches='tight') 
        # plt.show()
        """dumping all"""
        all_store.dump()